  ...
  async function handlePay() {
    ...
    try {
      ...
      const paid = await api.payOrder(
        order.id,
        payMethod === 'card' ? 'card' : 'cash',
        payMethod === 'cash' ? Number(cashReceived) : undefined
      );
      setSuccess(true);
      setChangeDue(paid.change_due ?? null);
      setLines([]);
      setCashReceived('');
      ...
      {success && (
        <>
          ...
          {changeDue != null && changeDue > 0 && (
            <div className="card">
              <div className="card__title">Change Due</div>
              <div style={{ fontSize: '2rem', fontWeight: 700 }} data-testid="change-due">${changeDue.toFixed(2)}</div>
            </div>
          )}
          <button className="btn btn--primary" style={{ marginBottom: 12 }} onClick={handleNewOrder}>New Order</button>
        </>
      )}

      {/* Order summary — always visible when items are in the order */}
      {lines.length > 0 && (
        <div className="card" data-testid="order-lines">
          ...
          {/* Inline payment */}
          <div className="tabs-nav" style={{ marginTop: 12 }}>
          ...
          <button data-testid="confirm-payment-btn" className="btn btn--success" style={{ marginTop: 8 }}
